...
//      digitalWrite(buzzer, LED_OFF);  // Make sure led is off
//      // Give some feedback
//      Serial.println(F("Wipe Button Pressed"));
  ...
  if (programMode) {
    ...
    if ( isMaster(readCard)) {    // If scanned card's ID matches Master Card's ID - enter program mode
      ...
      if ( findID(readCard)) { // If not, see if the card is in the EEPROM
        ...
        lcd.setCursor(0, 3);
        lcd.print("UNLOCKED");
        //granted(lockdelay);         // Open the door lock for lockdelay value
      }
...
void readsensor(){
  lockread = digitalRead(relay);
  state = digitalRead(Lswitch); 
  }
void readState(){
  readsensor();
  if(state == LOW){
...
//  if(state == 0 && lockread != 1){
//    lcd.init();
//    lcd.setCursor(4, 0);
//    lcd.print("ERROR!!!");
//    lcd.setCursor(3, 1);
//    lcd.print("DOOR OPEN!");
//    Serial.println("Error !!! Door is open");
//    delay(1000);
//  }
}
/////////////////////////////////////////  Access Granted    ///////////////////////////////////
void granted ( uint16_t setDelay) {
  ...
  digitalWrite(buzzer, LED_ON);   // Turn on green LED
  digitalWrite(relay, LOW);     // Unlock door!
  delay(state);          // Hold door lock open for given seconds
...
///////////////////////////////////////// Access Denied  ///////////////////////////////////
void denied() {
  digitalWrite(buzzer, LED_OFF);  // Make sure green LED is off
...
void cycleLeds() {
  ...
  digitalWrite(buzzer, LED_ON);   // Make sure green LED is on
  digitalWrite(blueLed, LED_OFF);   // Make sure blue LED is off
  delay(200);
  digitalWrite(redLed, LED_OFF);  // Make sure red LED is off
  digitalWrite(buzzer, LED_OFF);  // Make sure green LED is off
...
void successWrite() {
  digitalWrite(blueLed, LED_OFF);   // Make sure blue LED is off
  digitalWrite(redLed, LED_OFF);  // Make sure red LED is off
  digitalWrite(buzzer, LED_OFF);  // Make sure green LED is on
  delay(200);
  digitalWrite(buzzer, LED_ON);   // Make sure green LED is on
  delay(200);
  digitalWrite(buzzer, LED_OFF);  // Make sure green LED is off
